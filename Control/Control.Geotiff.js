...
// Note this will only work with ESPG:4326 tiffs
  ...
  try {
    ...
    let ImageDataPolyfill = function ImageDataPolyfill() {
  ...
  L.LeafletGeotiff = L.ImageOverlay.extend({
    ...
    getValueAtLatLng(lat, lng) {
      try {
        let x = Math.floor(this.raster.width * (lng - this._rasterBounds._southWest.lng) / (this._rasterBounds._northEast.lng - this._rasterBounds._southWest.lng));
        let y = this.raster.height - Math.ceil(this.raster.height * (lat - this._rasterBounds._southWest.lat) / (this._rasterBounds._northEast.lat - this._rasterBounds._southWest.lat)); // invalid indices
    ...
    _getPixelByLatLng(latLng) {
      ...
      let mercPoint = this._map.latLngToLayerPoint(latLng);
    ...
    _clipMaskToPixelPoints(i) {
      if (this.options.clip) {
        ...
        for (let p = 0; p < clip.length; p++) {
          let mercPoint = this._map.latLngToLayerPoint(clip[p]),
    ...
    _drawImage() {
      if (this.raster.hasOwnProperty("data")) {
        ...
        let size = this._map.latLngToLayerPoint(this._map.getBounds().getSouthEast())._subtract(topLeft);
        ...
        //let mask = this.createMask(size, args);
    ...
    createSubmask(size, args, clip) {
      ...
      for (let i = 0; i < clip.length; i++) {
        ...
        for (let j = 0; j < ring.length; j++) {
          let pix = this._getPixelByLatLng(ring[j]);
    ...
    createMask(size, args) {
      ...
      if (clip) {
        ...
        for (let idx = 0; idx < clip.length; idx++) {
          let submask = this.createSubmask(size, args, clip[idx]);
    ...
    transform(rasterImageData, args) {
      ...
      let imageData = new ImageData(args.plotWidth, args.plotHeight);
      let outData = imageData.data;
      let outPixelsU32 = new Uint32Array(outData.buffer);
      let inData = rasterImageData.data;
      let inPixelsU32 = new Uint32Array(inData.buffer);

      let zoom = this._map.getZoom();

      let scale = this._map.options.crs.scale(zoom);

      let d = 57.29577951308232; //L.LatLng.RAD_TO_DEG;

      let transformationA = this._map.options.crs.transformation._a;
      let transformationB = this._map.options.crs.transformation._b;
      let transformationC = this._map.options.crs.transformation._c;
      let transformationD = this._map.options.crs.transformation._d;
      ...
      for (let y = 0; y < args.plotHeight; y++) {
        let yUntransformed = ((args.yOrigin + y) / scale - transformationD) / transformationC;
        let currentLat = (2 * Math.atan(Math.exp(yUntransformed)) - Math.PI / 2) * d;
        let rasterY = this.raster.height - Math.ceil((currentLat - this._rasterBounds._southWest.lat) / args.latSpan);

        for (let x = 0; x < args.plotWidth; x++) {
          //Location to draw to
          let index = y * args.plotWidth + x; //Calculate lat-lng of (x,y)
          ...
          let xUntransformed = ((args.xOrigin + x) / scale - transformationB) / transformationA;
          let currentLng = xUntransformed * d;
          let rasterX = Math.floor((currentLng - this._rasterBounds._southWest.lng) / args.lngSpan);
          let rasterIndex = rasterY * this.raster.width + rasterX; //Copy pixel value
