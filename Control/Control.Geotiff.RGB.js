(function (factory) {
  ...
  L.LeafletGeotiff.RGB = L.LeafletGeotiffRenderer.extend({
    ...
    render: function (raster, canvas, ctx, args) {
      let rasterImageData = ctx.createImageData(raster.width, raster.height);
      let isGrayscale = raster.data.length < 3; // compute max band max value if not set yet
      ...
      let scaleMax = this.options.bandMaxVal > 0 ? this.options.bandMaxVal : 255;
      ...
      let imageData = this.parent.transform(rasterImageData, args);
      ctx.putImageData(imageData, args.xStart, args.yStart); // debug output

      /* let dPlotCanvas = document.getElementById("debugCanvas");
          dPlotCanvas.width = raster.width;
          dPlotCanvas.height = raster.height;
          let dCtx = dPlotCanvas.getContext("2d");
